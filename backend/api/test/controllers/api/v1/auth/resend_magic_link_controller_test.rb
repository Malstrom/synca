    ...
    module Auth
      class ResendMagicLinkControllerTest < ActionDispatch::IntegrationTest
        setup do
          ...
          @user.update!(magic_link_token: "old_token", magic_link_sent_at: 6.minutes.ago)
        end
        ...
        test "resend magic link with valid email" do
          ...
          assert_not_equal "old_token", @user.reload.magic_link_token
          assert @user.reload.magic_link_sent_at > 5.minutes.ago
        end
        ...
        test "resend magic link too soon" do
          ...
          assert_equal I18n.t("contracts.errors.email.invalid"), JSON.parse(response.body)["errors"].first
        end
      end
    end
  end
