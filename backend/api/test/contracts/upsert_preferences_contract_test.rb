...
class UpsertPreferencesContractTest < ActiveSupport::TestCase
  test "valid input" do
    ...
    assert_equal 4, result.to_h[:preferences][:sleep_together_importance]
    assert_equal "cool", result.to_h[:preferences][:temperature_preference]
    assert_equal "moderate", result.to_h[:preferences][:movement_preference]
    assert_equal 3, result.to_h[:preferences][:rhythm_importance]
    assert_equal "night", result.to_h[:preferences][:self_chronotype]
  end
  ...
  test "partial input" do
    result = UpsertPreferencesContract.new.call(
      preferences: { sleep_together_importance: 2, rhythm_importance: 4 }
    )

    assert result.success?
    assert_equal 2, result.to_h[:preferences][:sleep_together_importance]
    assert_equal 4, result.to_h[:preferences][:rhythm_importance]
    assert_nil result.to_h[:preferences][:temperature_preference]
    assert_nil result.to_h[:preferences][:movement_preference]
    assert_nil result.to_h[:preferences][:self_chronotype]
  end
end
