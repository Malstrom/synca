...
class UpsertPreferencesContractTest < ActiveSupport::TestCase
  ...
  test "invalid sleep_together_importance" do
    ...
    assert_includes result.errors[:preferences][:sleep_together_importance], "must be in 1..5"
  end
  ...
  test "invalid movement_preference" do
    ...
    assert_not result.success?
    assert_includes result.errors[:preferences][:movement_preference], "must be one of: very_little, moderate, a_lot, as_much_as_possible"
  end

  test "invalid rhythm_importance" do
    result = @contract.call(preferences: { rhythm_importance: 0 })
    assert_not result.success?
    assert_includes result.errors[:preferences][:rhythm_importance], "must be in 1..5"
  end
