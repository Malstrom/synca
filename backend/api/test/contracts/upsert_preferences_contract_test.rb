...
class UpsertPreferencesContractTest < ActiveSupport::TestCase
  ...
  test 'invalid sleep_together_importance' do
    ...
    assert_includes result.errors[:preferences].to_h[:sleep_together_importance], 'must be between 1 and 5'
  end

  test 'invalid temperature_preference' do
    result = @contract.call(preferences: { temperature_preference: 'invalid' })
    assert result.failure?
    assert_includes result.errors[:preferences].to_h[:temperature_preference], 'must be cool, warm or no_preference'
  end

  test 'invalid movement_preference' do
    result = @contract.call(preferences: { movement_preference: 'invalid' })
    assert result.failure?
    assert_includes result.errors[:preferences].to_h[:movement_preference],
  ...
  test 'invalid rhythm_importance' do
    ...
    assert result.failure?
    assert_includes result.errors[:preferences].to_h[:rhythm_importance], 'must be between 1 and 5'
  end

  test 'invalid self_chronotype' do
    result = @contract.call(preferences: { self_chronotype: 'invalid' })
    assert result.failure?
    assert_includes result.errors[:preferences].to_h[:self_chronotype], 'must be morning, night or depends'
  end
end
