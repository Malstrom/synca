...
class UpsertPreferencesContractTest < ActiveSupport::TestCase
  ...
  test "invalid sleep_together_importance" do
    ...
    assert_includes result.errors[:preferences][:sleep_together_importance], "must be between 1 and 5"
  end

  test "invalid temperature_preference" do
    result = @contract.call(preferences: { temperature_preference: "invalid" })
    assert_predicate result, :failure?
    assert_includes result.errors[:preferences][:temperature_preference], "must be cool, warm or no_preference"
  end

  test "invalid movement_preference" do
    result = @contract.call(preferences: { movement_preference: "invalid" })
    assert_predicate result, :failure?
    assert_includes result.errors[:preferences][:movement_preference], "must be very_little, moderate, a_lot or as_much_as_possible"
  end

  test "invalid rhythm_importance" do
    result = @contract.call(preferences: { rhythm_importance: 0 })
    assert_predicate result, :failure?
    assert_includes result.errors[:preferences][:rhythm_importance], "must be between 1 and 5"
  end

  test "invalid self_chronotype" do
    result = @contract.call(preferences: { self_chronotype: "invalid" })
    assert_predicate result, :failure?
    assert_includes result.errors[:preferences][:self_chronotype], "must be morning, night or depends"
  end
end
