...
class UpsertPreferencesContract < ApplicationContract
  params do
    required(:preferences).schema do
      optional(:sleep_together_importance).filled(:integer, in?: 1..5)
      ...
      optional(:rhythm_importance).filled(:integer, in?: 1..5)
      optional(:self_chronotype).filled(:string, included_in?: PreferenceProfile.self_chronotypes.keys)
    end
  end

  rule(:preferences) do
    if values[:preferences].keys.any?
      key.failure("must contain at least one preference") unless values[:preferences].keys.any?
    end
  end
end
