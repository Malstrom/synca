...
module Signals
  class SummaryPresenter
    ...
    def activity_tier
      ACTIVITY_TIER_LABELS[@health_summary.activity_level]
    end

    def self_report_alignment
      return nil unless @preference_profile

      if @preference_profile.self_chronotype == @health_summary.chronotype
        { aligned: true }
      else
        {
          aligned: false,
          note: "You declared \"#{CHRONOTYPE_LABELS[@preference_profile.self_chronotype]}\" but your data shows an average wake time of #{format_time(@health_summary.sleep_start_local)}."
        }
      end
    end
