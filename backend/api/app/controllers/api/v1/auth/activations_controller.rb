      ...
      class ActivationsController < ApplicationController
        ...
        def create
          ...
          in Success(attrs:)
            ...
            if user.nil?
              render_error(:not_found, I18n.t("controllers.auth.activations.not_found"))
            elsif user.magic_link_sent_at.nil? || user.magic_link_sent_at < 72.hours.ago
              render_error(:unprocessable_entity, I18n.t("controllers.auth.activations.token_expired"))
            elsif user.magic_link_token.nil?
              render_error(:unprocessable_entity, I18n.t("controllers.auth.activations.token_already_used"))
            elsif user.account_type == "active"
              render_error(:unprocessable_entity, I18n.t("controllers.auth.activations.account_already_active"))
            else
              user.update!(
                account_type: :active,
