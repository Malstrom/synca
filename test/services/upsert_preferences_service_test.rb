...
class UpsertPreferencesServiceTest < ActiveSupport::TestCase
  ...
  test "updates existing preference_profile on second call" do
    ...
    result1 = UpsertPreferencesService.call(
      current_user: user,
      attrs: { sleep_together_importance: 3 }
    )
    assert_predicate result1, :success?

    result2 = UpsertPreferencesService.call(
      current_user: user,
  ...
  test "partial update leaves untouched fields unchanged" do
    ...
    assert_predicate result, :success?
    updated_prefs = user.preference_profile.reload

    assert_equal 3, updated_prefs.sleep_together_importance
